...
// Combine all variant images + base product image for thumbnails

// const finalImages = [
//   detail?.product_image, // main product image
//   ...(variantDetail?.variant_multiple_image?.length
//       ? variantDetail.variant_multiple_image
//       : []),
//   ...(relatedProducts?.flatMap(p => p.product_images || [p.product_image]) || [])
// ].filter(Boolean); // remove any null/undefined

const finalImages = [
  detail?.product_image, 
  ...(variantDetail?.varients_multiple_image?.map(item => item.variant_image) || []),
  // ...(relatedProducts?.flatMap(p => p.product_images || [p.product_image]) || [])
].filter(Boolean);
...
console.log(detail?.product_variant_value_list?.[0]?.varients_multiple_image
,"avarient images")
  return (
    <>
    <div className="max-w-8xl mx-auto p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-3 gap-10">
...
<div className="w-full lg:h-[80vh] flex flex-col items-center">
  ...
  {/* Thumbnails */}
<div className="flex gap-3 mt-4 justify-center overflow-x-auto py-2 w-full px-2">
  {detail?.product_variant_value_list?.[0]?.varients_multiple_image?.map((img, index) => (
    <img
      key={index}
      onClick={() => setMainImage(img?.variant_image)}
